...
import { benchState, useLevelsState, teamsState } from '../states';
				...
				<Stack spacing={4}>
					...
					{teams.map((team, index) => (
						<Box p={4} borderWidth="1px" borderRadius="lg" overflow="hidden" key={`team-${index}`}>
							<Heading size="sm" mb={6}>
								<>Team {index + 1}</>
								{useLevels && <> ({teamSum(team)})</>}
							</Heading>
							<PlayerGroup group={team} />
						</Box>
					))}
				</Stack>
				<Button colorScheme="primary" size="lg" onClick={shuffle} leftIcon={<RepeatIcon />}>
					Shuffle teams
				</Button>
			</Stack>
		</AnimateSharedLayout>
